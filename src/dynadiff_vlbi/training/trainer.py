"""Minimal trainer for compact sequence reconstruction experiments."""
...
from dynadiff_vlbi.training.losses import compute_loss_dict
...
class Trainer:
    """Small trainer with checkpointing and CSV/JSON logs."""
    ...
    def _prepare_batch(self, batch: dict[str, torch.Tensor]) -> tuple[torch.Tensor, torch.Tensor]:
        inputs = batch["input"].to(self.device, non_blocking=True).unsqueeze(1)
    ...
    def _run_epoch(self, loader: DataLoader, training: bool) -> dict[str, float]:
        if training:
            ...
            self.model.eval()
        totals = {"total": 0.0, "reconstruction": 0.0, "temporal": 0.0}
        num_batches = 0
    ...
    def fit(self, train_loader: DataLoader, val_loader: DataLoader) -> dict[str, Any]:
        """Run the training loop and save best/latest checkpoints."""
