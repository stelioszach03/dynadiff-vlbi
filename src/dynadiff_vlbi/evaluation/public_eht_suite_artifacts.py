...
def select_public_qualitative_example(
    ...
    return {
        ...
        "prediction_path": repo_relative_path(chosen_spec.prediction_path(support_fraction_tag)),
        "per_sample_csv": repo_relative_path(chosen_spec.per_sample_path(support_fraction_tag)),
    }
            ...
            rows.append(
                {
                    ...
                    "emc_tto_heldout_visibility_rmse": _safe_float(
                        models.get("emc_tto", {}).get("heldout_visibility_rmse", float("nan"))
                    ),
                    "dps_heldout_visibility_rmse": _safe_float(
                        models.get("dps", {}).get("heldout_visibility_rmse", float("nan"))
                    ),
            ...
            rows.append(
                {
                    ...
                    "mean_emc_tto_heldout_visibility_rmse": _mean_support_metric(summary, "emc_tto", "heldout_visibility_rmse"),
                    "mean_dps_heldout_visibility_rmse": _mean_support_metric(summary, "dps", "heldout_visibility_rmse"),
...
def write_sensitivity_table(path: str | Path, rows: list[dict[str, Any]]) -> None:
    markdown_rows = [
        [
            ...
            _fmt(float(row["mean_emc_tto_heldout_visibility_rmse"])),
            _fmt(float(row.get("mean_dps_heldout_visibility_rmse", float("nan")))),
...
def build_release_robustness_rows(
    *,
    ...
    for baseline_summary in baseline_summaries:
        ...
        emc_tto_baseline_values = np.asarray(
            [_model_metrics(baseline_summary, tag, "emc_tto")["heldout_visibility_rmse"] for tag in baseline_support_tags],
            dtype=np.float64,
        )
        ...
        row = {
            ...
            "mean_emc_tto_baseline_track": float(np.nanmean(emc_tto_baseline_values)),
            "std_emc_tto_baseline_track": float(np.nanstd(emc_tto_baseline_values)),
            "emc_tto_uplift_baseline_track": float(np.nanmean(emc_baseline_values) - np.nanmean(emc_tto_baseline_values)),
            "mean_ehtim_baseline_track": float(np.nanmean(ehtim_baseline_values)),
            "mean_dps_baseline_track": _mean_support_metric(baseline_summary, "dps", "heldout_visibility_rmse"),
        ...
        if station_summary is not None:
            ...
            emc_tto_station_values = np.asarray(
                [_model_metrics(station_summary, tag, "emc_tto")["heldout_visibility_rmse"] for tag in station_support_tags],
                dtype=np.float64,
            )
            ...
            row.update(
                {
                    ...
                    "mean_emc_tto_station_dropout": float(np.nanmean(emc_tto_station_values)),
                    "std_emc_tto_station_dropout": float(np.nanstd(emc_tto_station_values)),
                    "emc_family_gap": float(np.nanmean(emc_station_values) - np.nanmean(emc_baseline_values)),
                    "emc_tto_family_gap": float(np.nanmean(emc_tto_station_values) - np.nanmean(emc_tto_baseline_values)),
...
def write_release_robustness_table(path: str | Path, rows: list[dict[str, Any]]) -> None:
    markdown_rows = [
        [
            ...
            _fmt(float(row["mean_emc_tto_baseline_track"])),
            _fmt(float(row.get("mean_dps_baseline_track", float("nan")))),
            _fmt(float(row["emc_tto_uplift_baseline_track"])),
            _fmt(float(row["std_emc_baseline_track"])),
            _fmt(float(row.get("mean_emc_station_dropout", float("nan")))),
            _fmt(float(row.get("mean_emc_tto_station_dropout", float("nan")))),
            _fmt(float(row.get("emc_family_gap", float("nan")))),
            _fmt(float(row.get("emc_tto_family_gap", float("nan")))),
    ...
    write_markdown_table(
        ...
        headers=[
            ...
            "Mean EMC-TTO (baseline-track)",
            "Mean DPS (baseline-track)",
            "EMC-TTO uplift",
            "Std EMC (baseline-track)",
            "Mean EMC (station-dropout)",
            "Mean EMC-TTO (station-dropout)",
            "EMC family gap",
            "EMC-TTO family gap",
            "Mean eht-imaging",
        ...
        notes=[
            "EMC-TTO uplift is EMC mean minus EMC-TTO mean on held-out visibility RMSE, so positive values mean test-time optimization helps.",
...
def build_public_stats_rows(
    *,
    ...
    specs = [
        ...
        ("Public EHT baseline-track suite", baseline_track_paths, "emc_tto", "emc", "heldout_visibility_rmse"),
        ("Public EHT baseline-track suite", baseline_track_paths, "emc_tto", "ccrr", "heldout_visibility_rmse"),
        ("Public EHT baseline-track suite", baseline_track_paths, "emc_tto", "residual_refinement", "heldout_visibility_rmse"),
        ("Public EHT baseline-track suite", baseline_track_paths, "emc_tto", "baseline_learned", "heldout_visibility_rmse"),
        ("Public EHT baseline-track suite", baseline_track_paths, "emc_tto", "ehtim_bridge", "heldout_visibility_rmse"),
        ("Public EHT baseline-track suite", baseline_track_paths, "emc_tto", "tikhonov", "heldout_visibility_rmse"),
        ("Public EHT baseline-track suite", baseline_track_paths, "emc_tto", "tikhonov", "heldout_reduced_chi2"),
        ...
        ("Public EHT station-dropout suite", station_dropout_paths, "emc_tto", "emc", "heldout_visibility_rmse"),
        ("Public EHT station-dropout suite", station_dropout_paths, "emc_tto", "ccrr", "heldout_visibility_rmse"),
        ("Public EHT station-dropout suite", station_dropout_paths, "emc_tto", "baseline_learned", "heldout_visibility_rmse"),
        ("Public EHT station-dropout suite", station_dropout_paths, "emc_tto", "ehtim_bridge", "heldout_visibility_rmse"),
        ("Public EHT station-dropout suite", station_dropout_paths, "emc_tto", "tikhonov", "heldout_visibility_rmse"),
    ]
    rows: list[dict[str, Any]] = []
...
def build_release_gap_rows(track_specs: list[PublicTrackSpec]) -> list[dict[str, Any]]:
    """Build release-level paired-gap rows for the main public baseline-track suite."""

    comparisons = [
        ("emc", "baseline_learned", "heldout_visibility_rmse"),
        ("emc", "residual_refinement", "heldout_visibility_rmse"),
        ("emc", "ccrr", "heldout_visibility_rmse"),
        ("emc_tto", "emc", "heldout_visibility_rmse"),
        ("emc_tto", "baseline_learned", "heldout_visibility_rmse"),
        ("emc_tto", "residual_refinement", "heldout_visibility_rmse"),
        ("emc_tto", "ccrr", "heldout_visibility_rmse"),
    ]
    rows: list[dict[str, Any]] = []
    for spec in track_specs:
        summary = load_json(spec.summary_path)
        track_label = _track_label(summary)
        for candidate_key, reference_key, metric_key in comparisons:
            candidate, reference = _collect_paired_values(
                per_sample_paths=spec.per_sample_paths,
                candidate_key=candidate_key,
                reference_key=reference_key,
                metric_key=metric_key,
            )
...
def write_release_gap_table(path: str | Path, rows: list[dict[str, Any]]) -> None:
    markdown_rows = [
        [
            str(row["track_label"]),
            str(row["comparison"]),
    ...
    write_markdown_table(
        ...
        rows=markdown_rows,
        title="Public EHT Release-Level Gap Summary",
        notes=[
            ...
            rows.append(
                {
                    ...
                    "emc_tto_mean_heldout_visibility_rmse": float(
                        np.nanmean(model_values.get("emc_tto", [float("nan")]))
                    ),
...
def write_day_band_table(path: str | Path, rows: list[dict[str, Any]]) -> None:
    markdown_rows = [
        [
            ...
            _fmt(float(row["emc_tto_mean_heldout_visibility_rmse"])),
    ...
    write_markdown_table(
        path,
        headers=["Track", "Support (%)", "Day", "Band", "Mean EMC", "Mean EMC-TTO", "Best model", "Best mean VisRMSE"],
        rows=markdown_rows,
    ...
    *,
    ...
    for axis, summary in zip(axes_array, summaries):
        for model_key in PLOT_MODEL_ORDER:
            if model_key not in summary["support_fractions"]["80"]["models"]:
                continue
            ...
            axis.plot(
                ...
                linestyle="--" if model_key == "emc_tto" else "-",
    ...
    legend_handles = [
        Line2D(
            ...
            lw=2.0,
            marker="o",
            linestyle="--" if key == "emc_tto" else "-",
            label=MODEL_LABELS[key],
        )
        for key in PLOT_MODEL_ORDER
        if any(key in summary["support_fractions"]["80"]["models"] for summary in summaries)
    ]
    fig.legend(
...
def save_transfer_gap_figure(
    *,
    ...
    release_gap_rows: list[dict[str, Any]],
    output_png: str | Path,
    output_svg: str | Path,
) -> None:
    """Save a synthetic-versus-public transfer-gap figure with release-level heterogeneity."""
    ...
    release_labels = [
        "M87 2017 (2019-D01-01)",
        "M87 2018 (2024-D01-01)",
        "3C279 2017 (2020-D01-01)",
        "CenA 2017 (2021-D03-01)",
    ]
    release_colors = {
        "M87 2017 (2019-D01-01)": "#b45309",
        "M87 2018 (2024-D01-01)": "#2563eb",
        "3C279 2017 (2020-D01-01)": "#7c3aed",
        "CenA 2017 (2021-D03-01)": "#059669",
    }
    x = np.arange(len(comparisons), dtype=np.float64)
    width = 0.34
    fig, axes = plt.subplots(1, 2, figsize=(12.8, 4.8), gridspec_kw={"width_ratios": [1.0, 1.3]})
    fig.patch.set_facecolor("white")
    synthetic_values = [synthetic.get(comp, {}).get("mean_delta", float("nan")) for comp in comparisons]
    pooled_values = [pooled_public.get(comp, {}).get("mean_delta", float("nan")) for comp in comparisons]

    left = axes[0]
    left.bar(x - width / 2.0, synthetic_values, width=width, color="#dc2626", label="Synthetic breadth")
    left.bar(x + width / 2.0, pooled_values, width=width, color="#2563eb", label="Pooled public baseline-track")
    left.axhline(0.0, color="#111827", linewidth=1.0, alpha=0.7)
    left.set_xticks(x)
    left.set_xticklabels(["vs Baseline", "vs Residual", "vs CCRR"])
    left.set_ylabel("Direction-aware mean delta\n(positive means EMC is better)", fontsize=10)
    left.set_title("Pooled synthetic versus pooled public", fontsize=12)
    left.grid(axis="y", alpha=0.22)
    left.legend(frameon=False, fontsize=8)

    right = axes[1]
    right.axhline(0.0, color="#111827", linewidth=1.0, alpha=0.7)
    offsets = np.linspace(-0.18, 0.18, len(release_labels))
    release_lookup = {(row["track_label"], row["comparison"]): row for row in release_gap_rows}
    for offset, track_label in zip(offsets, release_labels):
        values = [release_lookup.get((track_label, comp), {}).get("mean_delta", float("nan")) for comp in comparisons]
        right.scatter(
            x + offset,
            values,
            s=42,
            color=release_colors[track_label],
            label=track_label,
            zorder=3,
        )
    right.scatter(x, pooled_values, s=64, marker="D", color="#111827", label="Pooled public", zorder=4)
    right.set_xticks(x)
    right.set_xticklabels(["vs Baseline", "vs Residual", "vs CCRR"])
    right.set_title("Public release-level heterogeneity", fontsize=12)
    right.grid(axis="y", alpha=0.22)
    right.legend(frameon=False, fontsize=7, loc="best")

    fig.suptitle("Synthetic-to-public transfer gap on held-out visibility RMSE", fontsize=13)
    ...
    return [
        {
            ...
            "evidence": repo_relative_path(public_matrix_path),
        },
        {
            ...
            "evidence": repo_relative_path(stats_path),
        },
        {
            "claim": "The public transfer picture is heterogeneous across releases rather than explained by one pooled real-data number alone.",
            "evidence": repo_relative_path(release_gap_path),
        },
        {
            ...
            "evidence": repo_relative_path(sensitivity_path),
        },
        {
            ...
            "evidence": repo_relative_path(day_band_path),
        },
    ]
